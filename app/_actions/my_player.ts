...
export default class MyPlayer extends Player {
    ...
    update(playerMovement: PlayerMovement, keys: NavKeys, keyE: Phaser.Input.Keyboard.Key, keyR: Phaser.Input.Keyboard.Key, network: Network): void {
        if (!keys) {
            return
        }
        ...
        if (Phaser.Input.Keyboard.JustDown(keyR)) {
            //item 생성되면 computer, whiteboard, coffeemachine dialog띄워주기
        }
        if (this.playerBehavior === PlayerBehavior.STANDING) {
            //의자 찾으면 앉도록 playerBehavior 바꿔주기
            if (Phaser.Input.Keyboard.JustDown(keyE) && item?.itemType === Items.CHAIR) {
            ...
            if (Phaser.Input.Keyboard.JustDown(keyE)) {
                ...
                if (this.anims.currentAnim) {
                    network.updatePlayer(this.x, this.y, this.anims.currentAnim.key)
                } else { console.log('no animation available') }
            }
        }
    }
...
Phaser.GameObjects.GameObjectFactory.register(
    ...
    function (
        ...
        sprite.body?.setSize(sprite.width * 0.5, sprite.height * 0.2).setOffset(sprite.width * 0.5 * 0.5, sprite.height * 0.8)
