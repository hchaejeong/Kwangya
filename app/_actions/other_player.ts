...
export default class OtherPlayer extends Player {
    ...
    makeCall(myPlayer: MyPlayer, webRTC: WebRTC) {
        ...
        if (!this.connected && this.bufferTime >= 750 && myPlayer.readyToConnect && this.readyToConnect && myPlayer.videoConnected && myPlayer.playerId > this.playerId) {
...
declare global {
    namespace Phaser.GameObjects {
        interface GameObjectFactory {
            otherPlayer(x: number, y: number, texture: string, id: string, name: string, frame?: string | number): OtherPlayer
        }
    }
}
